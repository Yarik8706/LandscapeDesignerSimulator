...
// import { loadSession, saveSession, Turn } from "../services/sessions";
// import { summarizeFlow } from "./summarize";
//
// export const clientChatFlow = ai.defineFlow(
//     {
//   name: "clientChatFlow",
...
//
//       const systemPrompt =
//           `Ты играешь роль клиента ландшафтного дизайна. Общайся кратко и по делу.
...
//         ...historyForModel.lastK.map(t => ({role: t.role as any, content: t.text})),
//         {role: "user", content: input.message}
//       ];
//
//     
//       const run = async (model: string) => ai.generate({
//         messages: msgs,
//         json: true,
//         schema: {
//           type: "object",
...
//       const outLite = await withCache(key, 30, async () => (await run("gemini-2.0-flash-lite")).output as any);
//
//       let final = outLite;
...
//         const key2 = {
//           m: "flash",
//           msgs: msgs.map(m => ({r: m.role, c: typeof m.content === "string" ? m.content : ""}))
//         };
//         final = await withCache(key2, 15, async () => (await run("gemini-2.0-flash")).output as any);
//       }
//
//       const newTurns: Turn[] = [
